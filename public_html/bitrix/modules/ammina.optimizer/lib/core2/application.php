...
class Application
{
	...
	protected $startTimeLazyOptimize = false;
	protected $endTimeLazyOptimize = false;
	protected $totalTimeLazyOptimize = false;

	protected $startTimeAntiLazyOptimize = false;
	protected $endTimeAntiLazyOptimize = false;
	protected $totalTimeAntiLazyOptimize = false;
	...
	protected $startTimeSendRequest = false;
	protected $endTimeSendRequest = false;
	protected $totalTimeSendRequest = false;

	protected $totalTimeRequestCompleteImage = false;
	protected $totalTimeRequestCompleteJs = false;
	protected $totalTimeRequestComplete = false;

	protected $maxTimeRequestComplete = false;
	...
	{
		global $APPLICATION;
		$this->maxTimeRequestComplete = intval(COption::GetOptionString('ammina.optimizer', 'maxtime_request_complete', 2));
		...
		$this->arOptions = \Ammina\Optimizer\SettingsTable::getSettings(SITE_ID);
		...
		$this->oLazyOptimizer = new Lazy($this->arOptions['category']['lazy']['groups'], $this->arOptions['category']['main']['groups']['other']['options']['MOVE_JS_BXSTAT_TOP'] === "Y");
	...
	public function doCheckAutocompositeUpdateCache(&$strContent)
	{
		$bResult = false;
		/**
		 * @todo добавить обработку при изменении страницы
		 */
	...
	public function EndContent(&$strContent)
	{
		...
		$this->startTimeSendRequest = microtime(true);
		$this->doSendStackRequest();
		$this->endTimeSendRequest = microtime(true);
		$this->totalTimeSendRequest = $this->endTimeSendRequest - $this->startTimeSendRequest;
		...
		if ($this->isShowStat()) {
			...
			$strContent = str_ireplace("</body", $strStatContent . '<link rel="stylesheet" href="/bitrix/css/ammina.optimizer/public.css?' . filemtime($_SERVER['DOCUMENT_ROOT'] . '/bitrix/css/ammina.optimizer/public.css') . '"></body', $strContent);
		}
	}
	...
	public function doCheckComponentCacheData(&$strContent)
	{
		...
		while ($iPos !== false) {
			...
			$arCritical = unserialize($arTmp[0], ['allowed_classes' => false]);
	...
	public function doOptimizeHtml(&$strContent)
	{
		...
		$this->startTimeAntiLazyOptimize = microtime(true);
		if ($this->arOptions['category']['lazy']['options']['ACTIVE'] === "Y") {
			$this->oLazyOptimizer->doOptimizeAntiLazy();
		}
		$this->endTimeAntiLazyOptimize = microtime(true);
		$this->totalTimeAntiLazyOptimize = $this->endTimeAntiLazyOptimize - $this->startTimeAntiLazyOptimize;
		...
		//$this->oDelayOptimizer->doOptimizeMain();
		$this->endTimeJsOptimize = microtime(true);
		$this->totalTimeJsOptimize = $this->endTimeJsOptimize - $this->startTimeJsOptimize;

		$this->startTimeLazyOptimize = microtime(true);
		if ($this->arOptions['category']['lazy']['options']['ACTIVE'] === "Y") {
			$this->oLazyOptimizer->doOptimize();
		}
		$this->endTimeLazyOptimize = microtime(true);
		$this->totalTimeLazyOptimize = $this->endTimeLazyOptimize - $this->startTimeLazyOptimize;
		...
		if (Helper::isCompositeEnabled()) {
			...
			foreach ($arAreas as $k => $oArea) {
				...
				if (isset($arCachedData['staticPart']) && amopt_strlen($arCachedData['staticPart']) > 0) {
					...
					StaticArea::applyCachedData($arCachedData);
	...
	public function doOptimizeAjaxComponent(&$strContent)
	{
		...
		if (amopt_strpos($strContent, '<') !== false && amopt_strpos($strContent, '</') !== false) {
			...
			if ($this->arOptions['category']['js']['options']['ACTIVE'] === "Y") {
				/**
				 * @todo Проработать возможность отключения
				 */
		...
		if ($this->iRequestType == self::REQUEST_TYPE_HTML) {
			...
			<div class="amoptpub">
				...
				<ul>
					...
					<li><?= Loc::getMessage("AMMINA_OPTIMIZER_PANEL_STAT_TIME_IMAGE_OPTIMIZE_REQUEST", array("#TIME#" => round($this->totalTimeRequestCompleteImage, 5))) ?></li>
					...
					<li><?= Loc::getMessage("AMMINA_OPTIMIZER_PANEL_STAT_TIME_JS_OPTIMIZE_REQUEST", array("#TIME#" => round($this->totalTimeRequestCompleteJs, 5))) ?></li>
					<li><?= Loc::getMessage("AMMINA_OPTIMIZER_PANEL_STAT_TIME_ANTILAZY_OPTIMIZE", array("#TIME#" => round($this->totalTimeAntiLazyOptimize, 5))) ?></li>
					<li><?= Loc::getMessage("AMMINA_OPTIMIZER_PANEL_STAT_TIME_LAZY_OPTIMIZE", array("#TIME#" => round($this->totalTimeLazyOptimize, 5))) ?></li>
					...
					<li><?= Loc::getMessage("AMMINA_OPTIMIZER_PANEL_STAT_TIME_SEND_REQUEST_COMPLETE", array("#TIME#" => round($this->totalTimeRequestComplete, 5))) ?></li>
					<li><?= Loc::getMessage("AMMINA_OPTIMIZER_PANEL_STAT_TIME_SEND_REQUEST", array("#TIME#" => round($this->totalTimeSendRequest, 5))) ?></li>
				</ul>
				<?/*<p>Память: <?= $this->totalMemory ?></p>*/ ?>
			</div>
			<?
	...
	{
		return $this->bSupportWebP;
	}

	protected function doCheckBrowserFeatures()
	{
		/**
		 * Проверка возможностей и поддержки разных особенностей браузерами
		 */
		...
		switch ($this->strBrowser) {
			...
			case "Firefox":
				...
				if (version_compare($this->strBrowserVersion, '57', '>=')) {
					//$this->arSupportHeaders[] = "preload";//Если стоят настройки браузера. По умолчанию false настройки. Поэтому выключаем поддержку
				}
				if (version_compare($this->strBrowserVersion, '39', '>=')) {
					$this->arSupportHeaders[] = "preconnect";
				}
...
//		}
	...
	public function doSendStackRequestWorkServer()
	{
		...
		if (!empty($this->arRequestStack)) {
			...
			if (amopt_strlen($strUrl) <= 0 || $urltime <= 0 || $urltime < (time() - 3600)) {
				...
				if ($arUrlResponse['recheckKey']) {
					COption::SetOptionInt("ammina.optimizer", "pt", time());
				}
				if ($arUrlResponse['result'] === "ok") {
			...
			if (amopt_strlen($strUrl) > 0) {
				$arFields['k'] = COption::GetOptionString("ammina.optimizer", "amminabx_apikey", "");
				$lk = COption::GetOptionString("ammina.optimizer", "ammina_lkey", "");
				$uq = COption::GetOptionString("ammina.optimizer", "ammina_uq", "");
				...
				if (amopt_strlen($lk) <= 0 || $lktime <= 0 || $lktime < (time() - 3600)) {
					...
					$lk = md5('BITRIX' . CUpdateClient::GetLicenseKey() . 'LICENCE');
					$uq = COption::GetOptionString('main', 'server_uniq_id', '');
					COption::SetOptionString("ammina.optimizer", "ammina_lkey", $lk);
					COption::SetOptionString("ammina.optimizer", "ammina_uq", $uq);
					COption::SetOptionString("ammina.optimizer", "ammina_lkeytime", time());
				}
				$arFields['l'] = $lk;
				$arFields['u'] = $uq;
				$arFields['version'] = "5.0";
				foreach ($this->arRequestStack as $kRequest => $vRequest) {
					$arRequest = $vRequest['request'];
	...
	public function addTimeRequestCompleteImage($time)
	{
		$this->totalTimeRequestCompleteImage += $time;
		$this->totalTimeRequestComplete += $time;
	}

	public function addTimeRequestCompleteJs($time)
	{
		$this->totalTimeRequestCompleteJs += $time;
		$this->totalTimeRequestComplete += $time;
	}

	public function isAllowSendRequestComplete()
	{
		if (defined("AMMINA_OPTIMIZER_AGENT_CHECK_IMAGE") && AMMINA_OPTIMIZER_AGENT_CHECK_IMAGE === true) {
			return true;
		}
		if ($this->maxTimeRequestComplete < $this->totalTimeRequestComplete) {
			return false;
		}
		return true;
	}

	public function addConvertedImage($strFromFile, $strToFile)
	{
		$this->convertedFiles['images'][$strFromFile] = [
			'toFile' => $strToFile,
			'converted' => $strFromFile !== $strToFile
		];
		$this->convertedFilesIndex['images'][$strToFile] = $strFromFile;
	}

	public function getOriginalConvertedImage($strFileName)
	{
		if (isset($this->convertedFilesIndex['images'][$strFileName])) {
			return $this->convertedFilesIndex['images'][$strFileName];
		}
		return $strFileName;
	}
}
