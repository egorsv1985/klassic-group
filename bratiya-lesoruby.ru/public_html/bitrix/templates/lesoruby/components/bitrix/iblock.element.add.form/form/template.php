...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	<div class="row align-items-center gy-3 py-2">
	<?=bitrix_sessid_post()?>
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		...
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
			...
			<div class="col-<?=$propertyID == 4 ? '12' : '6';?>">
				<?
				$PLACEHOLDER = '';
		...
		<div class="col-md-6">
			<button type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" class="btn btn-border px-0 d-block w-100">Отправить заявку</button>
		</div>
	</div>
</form>
</div>