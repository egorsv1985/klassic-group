	...
	<div class="row gy-3">
		...
		<? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])) : ?>
			...
			<? foreach ($arResult["PROPERTY_LIST"] as $propertyID) : ?>
				...
				?>
					<input class="form-control form__input bg-light rounded-2 border border-secondary text-secondary p-3 h-100" type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" id="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" value="<?= $value ?>" placeholder="" required>
		...
		<div class="">
			<button type="submit" class="btn fs-20 fw-500 px-3 py-3 btn-primary <?= (strpos($APPLICATION->GetCurPage(), 'zelenye-zabory')  !== false) ? ' btn--green' : '' ?> <?= (strpos($APPLICATION->GetCurPage(), 'paneli')  !== false) ? ' btn--gray' : '' ?> bg-gradient w-100 border-0" name="iblock_submit" value="Отправить">
				Отправить
			</button>
		</div>
	</div>
